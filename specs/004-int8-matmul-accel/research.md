...
## R42. The receiving half: `acc_dump_replay.py` + `acc_dump_replay_tb.py` (2026-08-20)

R41's fix split in two: fw-quant made `acc_test.c`'s test vector integer-deterministic and added
`dump_test_vector()` to print it (the producing half); this is the receiving half -- a validated
parser and a cocotb harness that replays a captured dump through the real RTL, so a hardware
transcript can be reproduced in simulation from CAPTURED bytes rather than a RECONSTRUCTED guess
at them. `acc_r27_gs16_data.py` (R40) was reaching for exactly this and could not be, because it
was regenerated on a host with different compiler flags than the board's build -- R41 explains why
that could never have closed the loop no matter how carefully the reconstruction was done.

### Format, coordinated directly with fw-quant (not relayed through team-lead)

Proposed a version tag + a checksum (team-lead's explicit requirement: "checkable, not merely
parseable" -- the R35/R39/R40 chain of transcripts being compared without either was the whole
problem one level up). fw-quant landed it independently and it matches byte-for-byte: `djb2_accum()`
(`h = ((h<<5)+h+b) & 0xFFFFFFFF`), folded across `xq, xs, wq, ws` in that order as raw memory
bytes (floats as their native-endian IEEE754 bit pattern), printed as one `checksum: %x` line.
djb2 over CRC32 specifically because CRC32 carries polynomial/reflection/init-value choices that
are easy to implement subtly differently on two independent sides -- exactly the failure class
this exercise is about. Cross-checked: compiled fw-quant's real `djb2_accum()` natively against a
known byte sequence and confirmed `acc_dump_replay.checksum32()` produces the bit-identical result
(`0xeb02ead4` both ways) -- not assumed from matching source text, verified by running both.

### `FemtoRV/TEST/acc_dump_replay.py`

Parses and validates a captured `dump_test_vector()` transcript (tolerates surrounding serial
noise -- a whole session capture can be handed to it directly, not a hand-trimmed excerpt).
`AccDumpError` on ANY structural problem: wrong format version, a section whose token count
disagrees with its declared count, a missing terminator, a missing checksum line, or a checksum
mismatch. Never returns partially-valid data with a warning -- a caller gets fully validated data
or an exception. Six self-tests (happy path, corrupted checksum, truncated section, missing
terminator, wrong version, surrounding noise) written and run BEFORE wiring into cocotb, per this
project's established order; caught one real bug this way (`_CHECKSUM_RE`'s `0x?` meant literal
`'0'` + optional `'x'`, not an optional `"0x"` prefix -- fixed to `(?:0x)?`).

### `FemtoRV/TEST/acc_dump_replay_tb.py`

`test_replay_from_dump`: loads a dump from `ACC_DUMP_PATH` (default: a SYNTHETIC fixture,
`acc_dump_fixtures/synthetic_v1.txt`, generated by `scratchpad/gen_synthetic_dump.py` replicating
fw-quant's integer-only `gen_test_vector()`/`pow2f_exact()` scheme so the harness has something
well-formed to exercise before a board capture exists -- clearly marked in the fixture's own
header as NOT hardware verification). Replays the validated bytes through the real `acc_top`/
`acc_mac` via the same synthetic-SDRAM-injection pattern `_row_interleave_case`/
`test_r27_gs16_replay` already established, and checks the result bit-exact against the strict
reference computed from those SAME captured bytes -- no regeneration step anywhere in this path.

Verified two ways: the synthetic fixture passes (16/16 rows bit-exact), and a deliberately
corrupted checksum (`sed` on the fixture's checksum line) fails LOUDLY through the actual cocotb
test -- `AccDumpError` propagates to `AssertionError`, the test reports FAIL, not a silent pass on
bad data. Full existing suite re-run clean: `acc_mac_tb` 10/10, `acc_unit_tb` 13/13,
`acc_reject_tb` 14/14, plus this new test 1/1.

Wired into `TEST/Makefile` as `MODULE=acc_dump_replay_tb`, reusing `acc_unit_tb`'s DUT stack
(`acc_unit_wrapper.v` + `acc_top.v` + `acc_mac.v` + `acc_weight_fetch.v` + `acc_regs.v` +
`muchtoremember_burst.v`) unchanged.

### Audit: does anything ELSE in this suite compare generated-vs-captured data?

Team-lead asked whether the "generated input vs captured input" distinction that invalidated four
`acc_test.c` transcripts applies anywhere else in `acc_unit_tb.py`. It does not, for a structural
reason: every other test in that file generates its comparison input and its RTL input from the
SAME Python call, in the SAME process, in the SAME test run -- there is no second, independently-
built reconstruction attempting to reproduce that data across a build boundary.

- `test_wq_64x64`/`test_w1_64x192`/`test_w2_192x64`/`test_classifier_64x512`: weight bytes are
  read directly from the real `model.q8.bin` checkpoint file (captured, not generated, by
  construction). Activation vectors are `random.Random(seed)`-generated, but the SAME Python call
  that produces them feeds both `act_write()` (the RTL's input) and `ref_matmul_row()` (the
  reference) directly -- no cross-language, cross-build reconstruction step exists to diverge.
- `test_row_interleave_at_gs_min`/`test_row_interleave_below_gs_min` (`_row_interleave_case`):
  fully synthetic weight AND activation data, same single-Python-generation-feeds-both-paths
  structure.
- `test_reject_*`: no data comparison at all (descriptor rejection only) -- not applicable.
- `test_r27_gs16_replay` (R40) is the ONE exception, and is exactly what `acc_dump_replay_tb.py`
  exists to supersede for this specific shape: `acc_r27_gs16_data.py` WAS a cross-build
  reconstruction (a host C recompilation trying to reproduce the board's exact bytes), which R41
  proved is fundamentally unable to guarantee byte-identity no matter how carefully done. It is
  left in place (its own honest-gap writeup in R40 is a real, useful record), but its result
  should not be treated as fully closing the gs=16 question the way a captured-data replay would.

Python's float32 semantics being IEEE754-deterministic and platform-independent (unlike C compiled
with an unspecified/default `-ffp-contract`) is what makes single-process generation safe here;
this is not a coincidence to keep relying on without stating it, which is why this audit is
recorded rather than just implied.

### Status

Ready and RTL-verified against the synthetic fixture; not yet run against a real board capture.
Team-lead will build once fw-quant's producing half lands, capture the dump, and hand it back --
