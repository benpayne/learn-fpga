...
- [X] T014 [US1] **EARLY DECISION GATE (FR-004a, FR-004b)** — read T013's figures against SC-002's thresholds (mean KL < 0.01 nats, top-1 agreement > 95%) and against the generated text, and record an explicit 8-bit go/no-go with its basis in `specs/004-int8-matmul-accel/research.md`. **This runs entirely on the host and blocks all RTL work.** If no-go, switch to 16-bit before writing any integer datapath — that change alters the basis of bit-exact verification, so taking it later is far more disruptive
- [X] T015 [US1] Port the quantized inference into `FemtoRV/FIRMWARE/llama2/runq.c` — no mmap, no host file I/O, weights already resident, static allocation at the fixed addresses from data-model entity 2, reusing feature 003's serial output and profiler
- [X] T016 [US1] Implement activation quantization in `FemtoRV/FIRMWARE/llama2/quantize.c`, matching upstream `quantize()` exactly so results compare bit-for-bit (research R2 — the activation vector is quantized too, which the design document had missed)
- [X] T017 [US1] Extend `FemtoRV/FIRMWARE/llama2/model_load.c` to autodetect format by magic — `0x616b3432` means Q8_0 with a 256-byte header, absence means feature 003's legacy 28-byte layout — and reject a mismatch loudly (research R4; a wrong-format load is the silent-wrong-output failure this project keeps guarding against)
- [X] T018 [US1] Update the memory map in `FemtoRV/FIRMWARE/llama2/model_load.h` for the quantized model's much smaller footprint (~0.3 MB versus 1.06 MB), keeping the 2 MB region and noting in the comment that region size, not memory, is what caps model capacity (research R5)
- [X] T019 [US1] Add per-operation timing for activation quantization to `FemtoRV/FIRMWARE/llama2/profile.c` as a new category, so T025 can measure the scalar cost this feature adds
- [X] T020 [US1] Build with `cd FemtoRV/FIRMWARE/llama2 && make all` and confirm it links clean and fits below the model region — **verified independently**: `runq.bin` 77,396 B at 0x800000-0x812E14 (7.38% of the 1 MB region, well below the 0x900000 weight base); `llama2.bin` still builds clean at 65,676 B, so the shared `model_load.c`/`profile.c` edits did not break the fp32 sibling. Link line confirmed to include all five objects (sdramstart, runq, model_load, profile, quantize) — the omitted-object failure mode caught earlier in this feature
- [ ] T021 [US1] Copy the quantized model to a FAT card and verify its header on the host with a hexdump against `q8_format.h` — **header verification DONE** (host): magic 0x616b3432, version 2, dim 64, hidden_dim 192, layers 5, heads 8, kv_heads 4, vocab 512, seq_len 512, shared_classifier 1, GS 64, padding all-zero, file 299,008 B. Both `dim % GS` and `hidden_dim % GS` are 0 — the reason hidden_dim was padded 172->192. **Remaining: the physical card copy**, which needs the board's SD card in a reader — *Session A*
- [ ] T022 [HW] [US1] Program the feature-003 minimal bitstream — rebuild it as `femtosoc_llm_soft.bit` with `NRV_IO_ACCEL` undefined; the original was overwritten by the R20 rename (research R27) — and run the quantized model with `(cd FemtoRV/FIRMWARE/llama2 && make upload_runq)`; confirm it loads and generates — *Session A*. **Deliberately NOT the accelerator bitstream**: this run is the golden reference later sessions are compared against, and it should not be measured on a build with 4.8% timing margin
...
- [X] T034 [US2] Implement `FemtoRV/RTL/ACCEL/acc_top.v` wiring MAC, fetch engine, register block, weight FIFO, activation BRAM and result BRAM. **Four constraints carried forward from T032/T033, each found the hard way:**
...
- [X] T036 [US2] Run `acc_unit_tb`; confirm bit-identical results and that the weight FIFO never underruns at full burst rate; record achieved words/cycle
...
- [X] T038 [US2] Verify descriptor rejection paths in simulation — `n % gs != 0`, unsupported `gs`, out-of-range `n`/`d`, oversized `d` — each producing its distinct error code and not starting (FR-009). **DONE.** `FemtoRV/TEST/acc_reject_tb.py` proves the `acc_regs.v` half: 14/14 pass, verified independently. But only **3 of the 6 error codes are computed by `acc_regs.v` at all** — `ERR_DIM`, `ERR_GS`, `ERR_FULL`. `ERR_RANGE`, `ERR_SLOT` and `ERR_MODE` reach it only as input pins from the control FSM, and a descriptor with `n=8192` (2x past `MAX_N`), `d=65535`, or `mode=3` is **pushed into the queue with no error** at this level — proven, not assumed. FR-009's "MUST NOT start" is therefore a property of the `acc_top`+`acc_regs` pair. **Now covered**: `acc_unit_tb.py` has six acc_top-level rejection tests, all passing — `ERR_RANGE` (oversized n, oversized d), `ERR_SLOT` (d beyond the slot, bad slot index) and `ERR_MODE` (both attention modes). Each asserts `PERF_CYCLES == 0`, which proves the operation never entered S_RUNNING rather than merely that an error bit was set. `ERR_MODE` especially — it exists precisely because `acc_top` once fell through silently on unimplemented modes, and that fix has never been tested
...
- [X] T047 [US3] **Regression (constitution Principle V, SC-014, FR-021)**: run `cd FemtoRV && make colorlight_i5.synth` and confirm resources, timing and behaviour match the T006 baseline. This is mandatory, not a formality — the arbitration change touches RTL the working profile depends on
...
- [X] T053 [HW] [US4] Program `femtosoc_llm.bit` and run `(cd FemtoRV/FIRMWARE/examples && make upload_acc_test)`; confirm the result matches the reference exactly — *Session B*. **Repeat the comparison many times and report whether any failure is stable or intermittent** (research R27): at 4.8% timing margin a consistent mismatch means a logic bug and an intermittent one means timing, they need opposite next steps, and on a single run they are indistinguishable
- [X] T054 [HW] [US4] Record `PERF_CYCLES` and `PERF_STALL` and compare achieved words/cycle against T036's simulated figure — *Session B*
- [X] T055 [HW] [US4] Verify at least two descriptor rejection paths and the abort path on real hardware via `FemtoRV/FIRMWARE/examples/acc_test.c`, recording results in `specs/004-int8-matmul-accel/research.md` — *Session B*
- [X] T056 [US4] Run `cd FemtoRV && make colorlight_i5.synth` again after the `RTL/femtosoc.v` changes and record the result against the T006 baseline (SC-014)
